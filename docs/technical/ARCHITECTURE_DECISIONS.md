...
Decision: **Preact**. Reconsider only if Phase 1 requires extensive `preact/compat`, fails required accessibility/testing behavior, or needs a justified React-only dependency. Plain DOM remains appropriate for disposable prototypes.
...
- Namespaced rotating localStorage envelope first; IndexedDB for logs or measured growth.
